...
def install_periods_schema(engine: Engine):
    ...
    - is_extendable: Added to period_kinds for flexible scheduling
    """
    with engine.begin() as conn:
        ...
        CREATE TABLE IF NOT EXISTS period_kinds (
            ...
            is_extendable INTEGER DEFAULT 0,     -- Can this period be extended (e.g. extra 10 mins)?
        ...
        if count == 0:
            ...
            default_kinds = [
                ('LECTURE_40', 'Lecture - 40 min', 'teaching', 40, '#2196F3', '📚', 0),
                ('LECTURE_50', 'Lecture - 50 min', 'teaching', 50, '#1E88E5', '📚', 0),
                ('LECTURE_60', 'Lecture - 60 min', 'teaching', 60, '#1976D2', '📚', 0),
                ('STUDIO_120', 'Studio - 2 hours', 'teaching', 120, '#9C27B0', '🎨', 1),
                ('STUDIO_180', 'Studio - 3 hours', 'teaching', 180, '#8E24AA', '🎨', 1),
                ('STUDIO_240', 'Studio - 4 hours', 'teaching', 240, '#7B1FA2', '🎨', 1),
                ('PRACTICAL_90', 'Practical - 90 min', 'teaching', 90, '#FF9800', '🔬', 0),
                ('PRACTICAL_120', 'Practical - 2 hours', 'teaching', 120, '#F57C00', '🔬', 0),
                ('THEORY_50', 'Theory - 50 min', 'teaching', 50, '#4CAF50', '📖', 0),
                ('THEORY_60', 'Theory - 60 min', 'teaching', 60, '#43A047', '📖', 0),
                ('BREAK_10', 'Short Break - 10 min', 'break', 10, '#FFD54F', '☕', 0),
                ('BREAK_15', 'Short Break - 15 min', 'break', 15, '#FFC107', '☕', 0),
                ('BREAK_20', 'Long Break - 20 min', 'break', 20, '#FFB300', '☕', 0),
                ('LUNCH_30', 'Lunch - 30 min', 'lunch', 30, '#FF5722', '🍽️', 0),
                ('LUNCH_45', 'Lunch - 45 min', 'lunch', 45, '#F4511E', '🍽️', 0),
                ('LUNCH_60', 'Lunch - 60 min', 'lunch', 60, '#E64A19', '🍽️', 0),
                ('ASSEMBLY_20', 'Assembly - 20 min', 'assembly', 20, '#607D8B', '🎤', 0),
                ('ASSEMBLY_30', 'Assembly - 30 min', 'assembly', 30, '#546E7A', '🎤', 0),
                ('GENERAL_30', 'General - 30 min', 'other', 30, '#9E9E9E', '⚙️', 0),
                ('GENERAL_45', 'General - 45 min', 'other', 45, '#757575', '⚙️', 0),
            ]
            
            for code, label, role, duration, color, icon, blockable in default_kinds:
                conn.execute(sa_text("""
                    INSERT INTO period_kinds 
                    (code, label, role, default_duration_min, color_hex, icon, is_blockable, is_extendable, created_by)
                    VALUES (:code, :label, :role, :duration, :color, :icon, :blockable, 0, 'system')
                """), {
                    'code': code, 'label': label, 'role': role, 
                    'duration': duration, 'color': color, 'icon': icon, 'blockable': blockable
                })
            
            log.info(f"✅ Seeded {len(default_kinds)} default period kinds")
    
    log.info("✅ Periods/Timegrid schema installed successfully with division and multi-year support")
