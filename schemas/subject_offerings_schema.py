...
def install_subject_offerings_table(engine: Engine):
    ...
    with engine.begin() as conn:
        ...
        CREATE TABLE IF NOT EXISTS subject_offerings (
            ...
            base_subject_code TEXT COLLATE NOCASE,
            topic_code_ay   TEXT COLLATE NOCASE,
            topic_name      TEXT,
            ...
            -- CO Weightage Type (NEW - Default 'equal')
            co_weightage_type TEXT NOT NULL DEFAULT 'equal' CHECK (co_weightage_type IN ('equal', 'weighted')),
        ...
        # 1. Add 'co_weightage_type' if missing (Required for new feature)
        if not _has_column(conn, "subject_offerings", "co_weightage_type"):
            _exec(conn, "ALTER TABLE subject_offerings ADD COLUMN co_weightage_type TEXT NOT NULL DEFAULT 'equal'")
            logger.info("🔧 Migrated subject_offerings: Added co_weightage_type column.")

        # 2. Add 'syllabus_template_id' if missing (Safety check)
        if not _has_column(conn, "subject_offerings", "syllabus_template_id"):
            _exec(conn, "ALTER TABLE subject_offerings ADD COLUMN syllabus_template_id INTEGER REFERENCES syllabus_templates(id) ON DELETE SET NULL")
            logger.info("🔧 Migrated subject_offerings: Added syllabus_template_id column.")

        # Indexes
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_ay_degree_year_term ON subject_offerings(ay_label, degree_code, year, term)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_subject_code ON subject_offerings(subject_code)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_status ON subject_offerings(status)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_division ON subject_offerings(division_code)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_cg ON subject_offerings(curriculum_group_code)")
...
def install_offerings_audit_table(engine: Engine):
    ...
    with engine.begin() as conn:
        ...
        # Audit Indexes
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_audit_offering_id ON subject_offerings_audit(offering_id)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_audit_action ON subject_offerings_audit(action, occurred_at DESC)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_audit_actor ON subject_offerings_audit(actor, occurred_at DESC)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_audit_ay ON subject_offerings_audit(ay_label, degree_code, year, term)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_audit_subject ON subject_offerings_audit(subject_code)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_offerings_audit_correlation ON subject_offerings_audit(correlation_id)")
...
def install_offerings_snapshots_table(engine: Engine):
    ...
    with engine.begin() as conn:
        ...
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_snapshots_offering ON subject_offerings_snapshots(offering_id, snapshot_number DESC)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_snapshots_active ON subject_offerings_snapshots(is_active_version)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_snapshots_expires ON subject_offerings_snapshots(expires_at)")
...
def install_offerings_approvals_table(engine: Engine):
    ...
    with engine.begin() as conn:
        ...
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_approvals_offering ON subject_offerings_approvals(offering_id)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_approvals_status ON subject_offerings_approvals(status, requested_at DESC)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_approvals_requester ON subject_offerings_approvals(requested_by)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_approvals_expires ON subject_offerings_approvals(expires_at)")
...
def install_offerings_freeze_table(engine: Engine):
    ...
    with engine.begin() as conn:
        ...
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_freeze_log_offering ON subject_offerings_freeze_log(offering_id, occurred_at DESC)")
...
def install_offerings_health_checks_table(engine: Engine):
    ...
    with engine.begin() as conn:
        ...
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_health_checks_type ON subject_offerings_health_checks(check_type, checked_at DESC)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_health_checks_status ON subject_offerings_health_checks(status, checked_at DESC)")
        _exec(conn, "CREATE INDEX IF NOT EXISTS idx_health_checks_next ON subject_offerings_health_checks(next_check_at)")
...
def install_offerings_views(engine: Engine):
    ...
    with engine.begin() as conn:
        ...
        SELECT 
            ...
            o.co_weightage_type,  -- ADDED THIS for CO weight toggle
