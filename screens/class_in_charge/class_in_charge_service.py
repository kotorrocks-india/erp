...
UPDATED: Integrated with batch_term_dates for automatic date population
"""
...
# ============================================================================
# HELPER: Get Term Dates from batch_term_dates
# ============================================================================
...
def get_term_dates_from_db(
    ...
    """
    Get term start and end dates directly from batch_term_dates table.
    
    Returns:
        Tuple of (start_date, end_date) or None if not found
    """
    try:
        # Calculate which batch this year belongs to
        ...
        # Year 1 students are newest batch, Year 2 are from previous year, etc.
        batch_code = str(start_year - (year - 1))
        ...
        with engine.begin() as conn:
            result = conn.execute(sa_text("""
                ...
                WHERE degree_code = :deg
                  ...
                  AND ay_code = :ay
                  AND year_of_study = :yr
                  AND term_number = :term
                LIMIT 1
            """), {
                "deg": degree_code,
                "batch": batch_code,
            ...
            if result:
                start = date.fromisoformat(result[0]) if isinstance(result[0], str) else result[0]
                end = date.fromisoformat(result[1]) if isinstance(result[1], str) else result[1]
                return (start, end)
    except Exception as e:
        log.error(f"Error fetching term dates from DB: {e}")
...
def validate_assignment_dates(
    ...
    UPDATED: Uses batch_term_dates directly for validation.
    ...
    # --- Validate against Term dates from batch_term_dates ---
    try:
        term_dates = get_term_dates_from_db(engine, ay_code, degree_code, year, term)
        
        if not term_dates:
            return False, f"Could not find term dates for {ay_code}, Year {year}, Term {term}. Please configure term dates first."
