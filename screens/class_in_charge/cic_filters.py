...
def fetch_term_dates(engine: Engine, degree_code: str, ay_code: str, year: int, term: int) -> Optional[Dict]:
    """Fetch term dates from batch_term_dates table."""
    query = """
    ...
    with engine.begin() as conn:
        ...
        table_check = conn.execute(sa_text("SELECT name FROM sqlite_master WHERE type='table' AND name='batch_term_dates'")).fetchone()
        if not table_check:
            return None
            
        result = conn.execute(sa_text(query), {
            "deg": degree_code, "ay": ay_code, "yr": year, "term": term
        }).fetchone()
