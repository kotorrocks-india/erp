...
FIXED: Merged duplicate function definitions and ensures all columns are selected.
"""
...
def fetch_offerings_for_filters(engine: Engine, filters: Dict) -> List[Dict]:
    """Fetch offerings based on all filters."""
    query = """
        SELECT 
            so.id,
            so.degree_code, 
    ...
    # Filter Logic:
    # If a specific filter is selected, we show records matching that filter OR records where that field is NULL.
    # (e.g., Selecting Program P1 shows P1 subjects AND Common subjects).
    ...
    with engine.begin() as conn:
        ...
        # Load into DataFrame to handle NaN/None consistently
...
def render_co_filters(engine: Engine) -> Tuple[Optional[int], Optional[Dict]]:
    ...
    with col1:
        ...
        if not academic_years:
            st.warning("⚠️ No academic years found.")
            st.info("💡 Go to **Academic Years** to configure the calendar.")
            return None, None
    ...
    try:
        ...
        # Build offering options WITHOUT NaN issues
